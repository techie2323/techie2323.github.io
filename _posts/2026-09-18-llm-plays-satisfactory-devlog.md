...
- **Fix the route planner.** Debug why `pathfind.py`'s A* search sometimes reports no route in cases that should be well within its search bounds, and handle the height-mismatch problem flying introduces for `LOOK_AT` accuracy.
...
- **Belts with real waypoints.** Confirmed that `FIRE` can add intermediate poles along a route, but a script has never yet planned and laid out a full multi-point belt autonomously.
- **Broader building-type coverage.** Pipes, trains, vehicles, and drones haven't been tested at all yet — the toolkit should generalize, but that's still an assumption, not a confirmed result.
